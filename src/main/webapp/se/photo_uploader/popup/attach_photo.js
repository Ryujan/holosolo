    ...
    function html5Upload() {	
    	...
    	sUploadURL= 'uploadFileHtml5.do'; 	//upload URL
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){ // html5에서는 ajax 통신을 사용했다. 이해가 매우 쉽다.
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		//alert("accessing to Controller"); // 마찬가지로 제대로 접근하므로 더 이상 체크하지 않는다.
 		var categoryPath = window.opener.parent.document.getElementById("categoryPath").value;
 		alert(categoryPath);
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : "../../../uploadFile.do?command=fileUploader&&categoryPath="+categoryPath,	//샘플 URL입니다.
 	        /*sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',*/	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 			sCallback : "se/photo_uploader/popup/callback.html",
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
 	    	...
 	    	success : function(oCustomEvent) { // 이 부분일 것 같은 이유는 업로드가 성공적으로 이루어졌기 때문에...
 	    		// 맞다 이부분임. 그리고 이 부분에서 확인을 눌렀을 때 발생하는 함수가 아래와 같고, 중요한 것은 setPhotoToEditor().
 	    		// alert("success");
 	    		...
 	    		// }
 	    		var aResult = []; 
 	    		aResult[0] = oCustomEvent.htResult; // 수정된 부분
 	    		//alert("aResult : " + aResult[0].callback_func); // 값을 가져오는데... 문제는 객체다..?!
 	    		// 찾았다. 객체 안이 아마 맵방식으로 저장되어있고, 당연하네.. JSON이니까.. 그래서 값을 get방식으로 가져올 수 있다.
 	    		setPhotoToEditor(aResult); // 느낌은 이 부분이 Editor로 upload하는 부분 같다. 위의 수정된 부분이 문제..
 	    		// 이 방식에서 위의 객체로 넘겨줘야 한다. 즉, 받는 부분에서 내가 수정해보면 될 것같다. 값은 확실하게 받아왔다.
 	    		// 이 setPhotoEditor()의 작동 부분(586라인)과 결과 리턴 부분은 어딘지 모르겠다.
 	    		//버튼 비활성화
 	    		goReadyMode();
 	    		oFileUploader.reset();
 	    		window.close();
	 ...
	 * ]
	 */
 	function setPhotoToEditor(oFileInfo){ // 현재 이놈은 배열로 받았다. 그래서 배열의 값을 넣어주어야 한다.
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			//스마트 에디터 플러그인을 통해서 넣는 방법 (oFileInfo는 Array)
			// alert(oFileInfo[0].callback_func); // 여기까지도 값의 이동이 확실하다..
			opener.nhn.husky.PopUpManager.setCallback(window, 'SET_PHOTO', [oFileInfo]);
			// 지금 이 부분이 문제다. 여기서 지금 window에 배열로 값을 넘겨준다. 그런데, 이 배열로 넘겨줄 때, 과연 제대로 작동하는가
			// 작동하지 않는가의 점검을 할 수가 없다. opener가 누군지 알 수가 없다... 또한 배열을 왜 []에 싸서 넘겨주는가?
			// []은 모르겠지만, 이 함수가 작동하는 부분은 hp_SE2M_AttachQuickPhoto.js의 8라인에서 사용됨.
